...
router.post('/checkout', tenantMiddleware(), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!priceId) { res.status(400).json({ error: 'Ongeldig plan' }); return; }
...
router.get('/subscription', tenantMiddleware(), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { tenantId } = getTenantContext();
    const result = await db.query(`SELECT plan_slug, billing_status FROM tenants WHERE id = $1`, [tenantId]);
...
router.post('/portal', tenantMiddleware(), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { tenantId } = getTenantContext();
    const result = await db.query('SELECT stripe_customer_id FROM tenants WHERE id = $1', [tenantId]);
...
router.post('/webhook', async (req: Request, res: Response, next: NextFunction) => {
  ...
  } catch { res.status(400).json({ error: 'Ongeldige webhook' }); return; }
