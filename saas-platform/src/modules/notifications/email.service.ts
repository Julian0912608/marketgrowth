...
<head>
  ...
  <title>Your daily MarketGrow briefing</title>
</head>
...
async function sendBriefingEmail(data: TenantBriefingData): Promise<void> {
  ...
  const res = await fetch(RESEND_ENDPOINT, {
    ...
    body: JSON.stringify({
      from:    FROM_EMAIL,
      to:      [data.email],
      subject: '📊 Your MarketGrow briefing — ' + new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long' }),
      html,
    }),
  });
  ...
  for (const tenant of tenantsResult.rows) {
    try {
      ...
      await sendBriefingEmail({
        ...
        email:      tenant.email,
        firstName:  tenant.first_name || 'there',
