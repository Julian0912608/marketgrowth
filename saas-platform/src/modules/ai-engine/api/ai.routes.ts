...
//
// Sprint 3c batch 2: GET /insights delegeert naar briefingsService.
// Memory injection (top-K cosine) zit in briefingsService, niet meer
// inline in deze route. tenant_briefings is nu de canonical store.
// ============================================================
...
// ── GET /api/ai/insights ──────────────────────────────────────
// Sprint 3c: delegeert naar briefingsService. Memory injection
// (top-K cosine van ai_memories) gebeurt binnen de service.
// Persisteert in tenant_briefings zodat email cron en analytics
// dezelfde briefing zien als het dashboard.
router.get('/insights', featureGate('ai-recommendations'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Redis cache check (snelste pad voor herhaalde dashboard hits)
    if (!force) {
    ...
    // Generate of read uit tenant_briefings
    const result = force
      ? await briefingsService.generateAndStore(tenantId, 'on_demand')
      : await briefingsService.getOrGenerateForToday(tenantId, 'on_demand');

    // Track usage alleen wanneer we een Sonnet call hebben gedaan
    if (!result.fromCache) {
    ...
    // Schrijf Redis cache met plan-TTL
    const cacheKey = `ai:insights:${tenantId}`;
    await cache.set(cacheKey, JSON.stringify(payload), CACHE_TTL[planSlug] || 3600);

    logger.info('ai.insights.served', {
      tenantId,
      planSlug,
      fromCache:    result.fromCache,
      memoriesUsed: result.memoriesUsed,
...
Price: EUR ${product.price_min || 0} | Units sold: ${product.units_sold} | Revenue: EUR ${parseFloat(product.total_revenue || 0).toFixed(0)}
